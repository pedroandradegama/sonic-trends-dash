    ...
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {history.map((item) => (
                <div key={item.id} className="p-3 border rounded-lg hover:bg-accent/50 cursor-pointer transition-colors" onClick={() => loadFromHistory(item)}>
      ...
      <Alert variant="default" className="border-amber-500/50 bg-amber-50 dark:bg-amber-950/20">
        ...
        <AlertDescription className="text-amber-800 dark:text-amber-200">
          ...
          <br />Não substitui julgamento médico.
          <br />Não inclua identificadores do paciente (nome, CPF, telefone, endereço).
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardContent className="pt-6 space-y-6">
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="confirm" className="text-sm font-medium cursor-pointer">Confirmo que removi identificadores do paciente.</Label>
          </div>
          ...
          {inputMode === 'structured' && caseText && (
            <div className="space-y-2">
              ...
              <div className="p-3 bg-muted/50 rounded-md text-sm">{caseText}</div>
              <p className="text-xs text-muted-foreground">Caracteres: {caseText.length} {caseText.length < 20 && '(mínimo 20)'}</p>
            </div>
          )}

          <div className="space-y-2">
            <Button onClick={() => handleGenerate(false)} disabled={isGenerateDisabled} className="w-full" size="lg">
              {requestStatus === 'loading' ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Gerando hipóteses...</>
              ) : (
                <><Sparkles className="h-4 w-4 mr-2" />Gerar hipóteses diagnósticas</>
              )}
            </Button>
      ...
      {requestStatus === 'success' && (
        <Alert className="border-green-500/50 bg-green-50 dark:bg-green-950/20">
          ...
          <AlertDescription className="text-green-800 dark:text-green-200">Análise gerada com sucesso.</AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
                  ...
                  <Card key={h.rank} className="border-l-4 border-l-primary">
                    <CardContent className="pt-4">
                      <div className="flex items-start gap-3">
                        <Badge variant="default" className="shrink-0">#{h.rank}</Badge>
                        <div className="space-y-3 flex-1">
                          <h4 className="font-semibold">{h.diagnosis}</h4>
                          <div className="space-y-2">
                            <div><p className="text-sm font-medium text-muted-foreground">Justificativa:</p><p className="text-sm">{h.justification}</p></div>
                            <div><p className="text-sm font-medium text-muted-foreground">Argumentos contra:</p><p className="text-sm">{h.arguments_against}</p></div>
                            {h.confirmation_questions.length > 0 && (
                              <div><p className="text-sm font-medium text-muted-foreground">Perguntas para confirmar:</p>
                                <ul className="list-disc list-inside text-sm space-y-1">
                                  {h.confirmation_questions.map((q, i) => <li key={i}>{q}</li>)}
                                </ul>
                              </div>
                            )}
                          </div>
            ...
            {result.red_flags.length > 0 && (
              <div className="space-y-2">
                <h3 className="font-semibold text-lg flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-destructive" />Red Flags</h3>
                <ul className="space-y-1">
                  {result.red_flags.map((flag, i) => <li key={i} className="flex items-center gap-2 text-destructive"><span className="w-1.5 h-1.5 rounded-full bg-destructive" />{flag}</li>)}
                </ul>
              </div>
            )}
            ...
            {result.next_steps.length > 0 && (
              <div className="space-y-2">
                ...
                <ul className="space-y-1">
                  {result.next_steps.map((step, i) => <li key={i} className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-primary" />{step}</li>)}
                </ul>
              </div>
            )}
